;;; FILE: "st-date-time.ss"
;;; IMPLEMENTS: PointInTime, DateAndTime, Duration
;;; AUTHOR: Ken Dickey
;;; DATE: 16 January 2017; April 2025
...
(define DateAndTime
  (newSubclassName:iVars:cVars:
   Magnitude
   'DateAndTime st-nil st-nil))
(rebase-mdict! DateAndTime st-date+time-behavior)

(define PointInTime
  (newSubclassName:iVars:cVars:
   Magnitude
   'PointInTime st-nil st-nil))
(rebase-mdict! PointInTime st-time-behavior)

(define Duration
  (newSubclassName:iVars:cVars:
   Magnitude
   'Duration st-nil st-nil))
(rebase-mdict! Duration st-duration-behavior)
...
(addSelector:withMethod:
     ...
     (lambda (self)
       ...
       (let* ( (offset-seconds (date-zone-offset self))
               ...
               (date-string    (format #f "~a" self)) ;;@@@(date->string self "~5"))
